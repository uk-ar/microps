...
struct tcp_queue_entry
{
    ...
    struct timeval last; // previous sending time
    unsigned int rto;    // micro seconds // timeout
    uint32_t seq;
    uint8_t flg;
...
static struct tcp_pcb *tcp_pcb_select(struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    for (pcb = pcbs; pcb < tailof(pcbs); pcb++)
    {
        if ((pcb->local.addr == IP_ADDR_ANY || pcb->local.addr == local->addr) && pcb->local.port == local->port)
        {
            if (!foreign)
            {
                // when called to check if enable to bind
                return pcb;
            }
...
static ssize_t tcp_output_segment(uint32_t seq, uint32_t ack, uint8_t flg, uint16_t wnd, uint8_t *data,
                                  size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
...
static ssize_t tcp_output(struct tcp_pcb *pcb, uint8_t flg, uint8_t *data, size_t len)
{
    ...
    {                   // First time sending
        seq = pcb->iss; // use initial sequence number
    }
    if (TCP_FLG_ISSET(flg, TCP_FLG_SYN | TCP_FLG_FIN) || len)
    {
        // Add only sequence number consuming segment, excluding ack or rst segment
                                ...
                                struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    case TCP_PCB_STATE_LISTEN:
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) // expected
        {
            // ignore security/compartment check
            // ignore precedence check
            pcb->local = *local;
            pcb->foreign = *foreign;
            pcb->rcv.wnd = sizeof(pcb->buf); // recievable window size
            pcb->rcv.nxt = seg->seq + 1;     // next sequece number for ack
            pcb->irs = seg->seq;
            pcb->iss = random();
            tcp_output(pcb, TCP_FLG_SYN | TCP_FLG_ACK, NULL, 0); // send SYN+ACK

            pcb->snd.nxt = pcb->iss + 1;             // sequence number to send next
            pcb->snd.una = pcb->iss;                 // last sequence number of which ACK is not returned
            pcb->state = TCP_PCB_STATE_SYN_RECEIVED; // move to SYN_RECEIVED
    ...
    case TCP_PCB_STATE_LAST_ACK:
        ...
        does not exceed the window. One could tailor actural segments to fit this assumption by trimming
        off any portions that lie outside the window (including SYN and FIN), and only processing further if
        the same segment then begins at RCV.NXT. Segments with highter beginning sequence number may be held
    ...
    case TCP_PCB_STATE_SYN_RECEIVED:
        if (pcb->snd.una <= seg->ack && seg->ack <= pcb->snd.nxt)
        {                                           // check if this ack is reasonable?
            pcb->state = TCP_PCB_STATE_ESTABLISHED; // connection established
            sched_wakeup(&pcb->ctx);                // wakeup waiting thread
        }
        else
        {
            tcp_output_segment(seg->ack, 0, TCP_FLG_RST, 0, NULL, 0, local, foreign);
    ...
    case TCP_PCB_STATE_CLOSE_WAIT:
        if (pcb->snd.una < seg->ack && seg->ack <= pcb->snd.nxt)
        {
            // Check if it is the ack to sending data which does not have ack
            pcb->snd.una = seg->ack;
            tcp_retransmit_queue_cleanup(pcb);
            // ignore: Users should recieve positive ackowledgements for buffers
            ...
            {
                // this ack is already checked
                // ignore
            }
            else if (seg->ack > pcb->snd.nxt)
            {
                // this ack is out of range
                tcp_output(pcb, TCP_FLG_ACK, NULL, 0);
                return;
            }
            switch (pcb->state)
    ...
    case TCP_PCB_STATE_FIN_WAIT2:
        if (len)
        {
            // copy recieved data to buffer and return ack
            memcpy(pcb->buf + (sizeof(pcb->buf) - pcb->rcv.wnd), data, len);
            pcb->rcv.nxt = seg->seq + seg->len;
            pcb->rcv.wnd -= len;
            tcp_output(pcb, TCP_FLG_ACK, NULL, 0);
            sched_wakeup(&pcb->ctx); // notify other thread
        }
        break;
    case TCP_PCB_STATE_CLOSE_WAIT:
    case TCP_PCB_STATE_LAST_ACK:
...
static void tcp_retransmit_queue_emit(void *arg, void *data)
{
    ...
    {
        // if retry too many times, discard connection
        pcb->state = TCP_PCB_STATE_CLOSED;
        sched_wakeup(&pcb->ctx);
        return;
    }
...
static void tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (len < (hdr->off) >> 2)
    {
        errorf("length error:len %zu, hdr->off=%u", len, hdr->off >> 2);
        return;
    }
    pseudo.src = src;
    pseudo.dst = dst;
    ...
    {
        errorf("can not handle broadcast");
        return;
    }
...
int tcp_open_rfc793(struct ip_endpoint *local, struct ip_endpoint *foreign, int active)
{
    ...
    else
    {
        debugf("passive open: local=%s, waiting for connection...",
               ip_endpoint_ntop(local, ep1, sizeof(ep1)));
        pcb->local = *local;
        if (foreign) // While socket API cannot listen, RFC793 spec allows to listen limited external addr
        {
            pcb->foreign = *foreign;
        }
        pcb->state = TCP_PCB_STATE_LISTEN; // change state to listen
    }
AGAIN:
    state = pcb->state;
    ...
    {
        if (sched_sleep(&pcb->ctx, &mutex, NULL) == -1) // wait for state change
        {
            debugf("interrupted");
            pcb->state = TCP_PCB_STATE_CLOSED;
            tcp_pcb_release(pcb);
...
RETRY:
    ...
    case TCP_PCB_STATE_ESTABLISHED:
    case TCP_PCB_STATE_CLOSE_WAIT:
        iface = ip_route_get_iface(pcb->foreign.addr); // iface for send
        if (!iface)
        {
...
RETRY:
    ...
    case TCP_PCB_STATE_ESTABLISHED:
        ...
        if (!remain) // Wait until data copied to recieve buffer
        {
            if (sched_sleep(&pcb->ctx, &mutex, NULL) == -1)
            {
                debugf("interrupted");
    ...
    len = MIN(size, remain);
    memcpy(buf, pcb->buf, len);                      // copy truncated data to buffer
    memmove(pcb->buf, pcb->buf + len, remain - len); // delete copied data
    pcb->rcv.wnd += len;
    mutex_unlock(&mutex);
    return len;
}
