...
static int ether_tap_addr(struct net_device *dev)
{
    ...
    soc = socket(AF_INET, SOCK_DGRAM, 0); // in order to get HWADDR
    if (soc == -1)
    {
    ...
    if (ioctl(soc, SIOCGIFHWADDR, &ifr) == -1) // get HWADDR
    {
        errorf("ioctl [SIOCGIFHWADDR]: %s,dev=%s", strerror(errno), dev->name);
        close(soc);
        return -1;
    }
    memcpy(dev->addr, ifr.ifr_hwaddr.sa_data, ETHER_ADDR_LEN); // copy HWADDR
    close(soc);
    return 0;
}
...
static int ether_tap_open(struct net_device *dev)
{
    ...
    strncpy(ifr.ifr_name, tap->name, sizeof(ifr.ifr_name) - 1); // name of tap device
    ifr.ifr_flags = IFF_TAP | IFF_NO_PI;                        // no packet information
    if (ioctl(tap->fd, TUNSETIFF, &ifr) == -1)
    { // request tap device creation
        errorf("ioctl[TUNSETIFF]:%s,dev=%s", strerror(errno), dev->name);
        return -1;
    }

    // for signal handling,which raise signal when data ready
...
static int ether_tap_transmit(struct net_device *dev, const uint16_t type,
...
static int ether_tap_isr(unsigned int irq, void *id)
{
    ...
    while (1)
    {
        ret = poll(&pfd, 1, 0); // time out 0 (try poll)
        if (ret == -1)
        {
            if (errno == EINTR)
            {
...
struct net_device *ether_tap_init(const char *name, const char *addr)
{
    ...
    tap->fd = -1; // invalid value as initial value;
