...
static void ether_dump(const uint8_t *frame, size_t flen)
{
    ...
    flockfile(stderr);
    fprintf(stderr, "   src: %s\n", ether_addr_ntop(hdr->src));
    fprintf(stderr, "   dst: %s\n", ether_addr_ntop(hdr->dst));
                          ...
                          ether_transmit_func_t callback)
{
    ...
    size_t flen, padding = 0;

    hdr = (struct ether_hdr *)frame;
    memcpy(hdr->dst, dst, ETHER_ADDR_LEN);
    memcpy(hdr->src, dev->addr, ETHER_ADDR_LEN);
    hdr->type = hton16(type);
    memcpy(hdr + 1, data, len); // why hdr+1?
    if (len < ETHER_PAYLOAD_SIZE_MIN)
    {
        padding = ETHER_PAYLOAD_SIZE_MIN - len;
    }
    flen = sizeof(*hdr) + len + padding;
...
int ether_input_helper(struct net_device *dev, ether_input_func_t callback)
{
    ...
    flen = callback(dev, frame, sizeof(frame)); // read ethernet frame
    ...
    hdr = (struct ether_hdr *)frame;
    // if dest is not device address and not broad
